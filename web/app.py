...
def run_engine(zip_path: Path):
    if Pipeline is None:
        raise RuntimeError(f"Não foi possível carregar o motor do LicitaSuite: {PIPELINE_ERROR}")

    pipeline = Pipeline()

    if hasattr(pipeline, "run"):
        return pipeline.run(str(zip_path))
    if hasattr(pipeline, "execute"):
        return pipeline.execute(str(zip_path))
    if hasattr(pipeline, "process"):
        return pipeline.process(str(zip_path))

    raise RuntimeError("O motor do LicitaSuite não possui método run, execute ou process.")
...
def main():
    ...
    with col2:
        ...
        st.caption("Usa o mesmo motor da versão desktop.")
    ...
    if uploaded is None:
        ...
        if st.button("⚙️ GERAR ATAS", type="primary", use_container_width=True):
            ...
            try:
                ...
                progress.progress(20, text="Carregando motor do LicitaSuite...")
                st.write("✓ Motor carregado")
                ...
                if not generated:
                    ...
                    st.error("O motor terminou, mas nenhuma Ata DOCX foi localizada.")
                    return
