...
try:
    ...
    build_status_data = None
    create_control_workbook = None
    add_extra_files_to_zip = None
...
def collect_generated_docx():
    ...
    for folder in [
        ATAS_DIR,
        OUTPUT_DIR,
        BASE_DIR / "atas_geradas",
        BASE_DIR / "output" / "atas_geradas",
    ]:
        if folder.exists():
...
.ls-logo img{width:min(300px,28vw);min-width:220px;height:auto;display:block;}
...
.ls-hero{text-align:center;padding:34px 0 30px 0;}
...
def render_header():
    ...
    st.markdown(
        f'<div class="ls-header-right"><span>{APP_TITLE}</span><span class="ls-pill">Uso Interno</span></div></header>',
        unsafe_allow_html=True,
    )
...
def render_preview(status_data):
        ...
        <div class="ls-preview">
            ...
            <div class="ls-preview-grid">
                <div class="ls-mini-card">
                    ...
                    <div style="color:#566482;">Aguardando processamento.</div>
                </div>
    ...
    for idx, f in enumerate(fornecedores, start=1):
        ...
        linhas.append(
            f'<div class="ls-example-line"><strong>ATA {idx:03d}</strong><span>{nome} — Itens: {itens_txt or "-"}</span></div>'
        )
...
def main():
    ...
    with col_upload:
        st.markdown(
            ...
            <div class="ls-card">
                <div class="ls-card-title-row">
                    ...
                    <div>
                        <h2>1. Enviar processo</h2>
                        <p>Selecione o arquivo ZIP do processo.</p>
                    </div>
                </div>
                <div class="ls-upload-area">
                    <div class="ls-upload-title">Arraste e solte o arquivo aqui</div>
                    <div class="ls-upload-sub">ou clique para selecionar</div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            """
                    <div class="ls-upload-meta">Formato: ZIP • Tamanho máximo: 200MB</div>
                </div>
        ...
        if generate_clicked:
            ...
            try:
                ...
                if not files:
                    ...
                    if build_status_data is not None:
                        try:
                            status_data = build_status_data(result, OUTPUT_DIR, files)
                            st.session_state.status_data = status_data
                        except Exception as status_exc:
                            st.warning(f"Atas geradas. A prévia inteligente não foi concluída: {status_exc}")

                    if status_data and create_control_workbook is not None:
                        try:
                            controle_path = create_control_workbook(status_data, OUTPUT_DIR)
                        except Exception as controle_exc:
                            st.warning(f"Atas geradas. A planilha de controle não foi gerada: {controle_exc}")

                    if controle_path and add_extra_files_to_zip is not None:
                        try:
                    ...
                    if status_data:
                        st.info(
                            f"Resumo: {status_data.get('total_atas', len(files))} ata(s), "
